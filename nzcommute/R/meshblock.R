...
#'
#' Reads a meshblock shapefile and optionally cleans one population-weight column
#' for population-weighted sampling.
#'
#' @param meshblock_file Path to a meshblock shapefile.
#' @param target_crs Target CRS, typically the CRS of the SA2 shapefile.
#' @param meshblock_id_col Optional meshblock ID column to validate.
#' @param population_col Optional population-weight column to clean.
#' @return A cleaned sf object transformed to `target_crs`.
#' @export
prepare_meshblock_population <- function(meshblock_file,
                                         target_crs,
                                         meshblock_id_col = NULL,
                                         population_col = NULL) {
  mb <- sf::st_read(meshblock_file, quiet = TRUE)

  if (!is.null(meshblock_id_col) && !meshblock_id_col %in% names(mb)) {
    stop("Meshblock shapefile does not contain ", meshblock_id_col, call. = FALSE)
  }
  if (!is.null(population_col) && !population_col %in% names(mb)) {
    stop("Meshblock shapefile does not contain ", population_col, call. = FALSE)
  }
  ...
  if (!is.null(population_col)) {
    mb <- mb |>
      dplyr::mutate(
        "{population_col}" := ifelse(
          is.na(.data[[population_col]]) | .data[[population_col]] < 0,
          0,
          .data[[population_col]]
        )
      )
...
create_population_weighted_sampler <- function(meshblock_sf,
  ...
  function(sa2_code) {
    ...
    data.frame(
      lon = as.numeric(xy[1, 1]),
      lat = as.numeric(xy[1, 2]),
      mb_code = if (!is.null(meshblock_id_col) && meshblock_id_col %in% names(poly)) as.character(poly[[meshblock_id_col]][1]) else NA_character_,
